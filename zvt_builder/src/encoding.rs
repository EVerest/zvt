...
mod test {
    ...
    fn test_tag_default_encode_decode() {
        ...
        assert_eq!(
            Default::decode(&[0x23, 0x01]).unwrap(),
            (Tag(0x23), &[0x01][..])
        );
        ...
        assert_eq!(
            Default::decode(&[0x1f, 0x80, 0x00, 0xab]).unwrap(),
            (Tag(0x8000), &[0xab][..])
        );

        assert_eq!(Default::encode(&Tag(0x8001)), [0x1f, 0x80, 0x01]);
        assert_eq!(
            Default::decode(&[0x1f, 0x80, 0x01]).unwrap(),
            (Tag(0x8001), &[][..])
        );

        // 2-byte 0x1f80 must not be parsed as 3-byte prefix.
        assert_eq!(
            Default::decode(&[0x1f, 0x80, 0x99]).unwrap(),
            (Tag(0x1f80), &[0x99][..])
        );
